...
def calculate_and_save_profile(
    ...
    """
    Computes clinical TDEE and macronutrient distributions (Mifflin-St Jeor),
    upserts into Supabase health_profiles, and synchronizes the in-memory UserCache.
    """
    supabase = get_supabase_admin_client()

    # If gender is omitted, attempt to retrieve previous gender from DB
    if not gender:
        try:
            prev = supabase.table('health_profiles').select('gender').eq('user_id', user_id).maybe_single().execute()
            if prev and prev.data and prev.data.get('gender'):
                gender = prev.data['gender']
        except Exception:
            gender = 'male'

    # Sanitize Gender for Postgres check constraint: ('male', 'female', 'other')
    raw_gender = (gender or 'male').lower()
    if raw_gender not in ('male', 'female', 'other'):
        raw_gender = 'other'

    # Sanitize Goal for Postgres check constraint: ('fat_loss', 'muscle_gain', 'maintenance')
    raw_goal = (goal or 'maintenance').lower()
    ...
    raw_act = (activity_level or 'sedentary').lower()
    if 'very' in raw_act:
        db_act = 'very_active'
    elif 'mod' in raw_act:
    ...
    conditions = list(medical_conditions or [])
    ...
    daily_protein_g = max(25, int(weight_kg * 2))
    ...
    # Update In-Memory User Cache for zero-latency AI Coach & Workout lookups
    user_cache.set_profile(user_id, db_payload)
    ...
    return {
        ...
        "daily_carbs_g": daily_carbs_g,
        "daily_fat_g": daily_fat_g,
        "data": response.data if response else db_payload
    }
...
def create_health_profile(data: OnboardingData, authenticated_user_id: str = Depends(get_current_user_id)):
    ...
    try:
        return calculate_and_save_profile(
...
@router.post("/update")
@router.put("/update")
def update_health_profile(data: ProfileUpdateData, authenticated_user_id: str = Depends(get_current_user_id)):
    if data.user_id != authenticated_user_id:
        raise HTTPException(status_code=403, detail="Forbidden: You do not own this resource")
